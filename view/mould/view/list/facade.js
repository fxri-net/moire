 ...
 * 云纹模具-视图-列表
 */
fxView['mould']['view']['list'] = function() {
    // 初始化变量
    var dark = {
        // 基础
            ...
            'done': function(data, curr, count) {
                ...
                if (self != top) {
                    url = fxBase['dom']['url']({
                        ...
                        'param': {
                            'data': {
                                'frame': encodeURIComponent(encodeURIComponent(url))
                            }
                        }
                    });
                    top.window.history.replaceState(null, null, url);
                }
                // 初始化layui设定
    ...
    // 执行视图-搜索
    fxView['mould']['view']['search'](dark['search']);
    ...
    // 执行视图-表格
    // 疏理数据
    tray['list'] = fxView['deploy']['cache']['table'] = {};
    $.each(dark['table']['cols'], function(key, value) {
        $.each(value, function(key2, value2) {
            ...
            // 解析数据
            value2['extend']['id'] = fxBase['base']['explode'](',', value2['extend']['id']);
            value2['extend']['type'] = fxBase['base']['explode'](',', value2['extend']['type']);
            $.each(value2['extend']['type'], function(key3, value3) {
                // 校验元素
                if (!isFunction(fxView['material']['elem'][value3])) return true;
                // 配置数据
                ...
                tray['data']['pack'] = value2;
                tray['data']['id'] = !isNull(value2['extend']['id'][key3]) ? value2['extend']['id'][key3] : key + '-' + value2['field'] + '-' + key3;
                tray['data']['type'] = value3;
                tray['list'][tray['data']['id']] = fxView['material']['elem'][value3]();
    ...
    $.each(tray['list'], function(key, value) {
        ...
        value['deploy']();
        // 设置模板
        value['dark']['pack']['templet'] = value['dark']['templet'];
    });
    // 绘制界面
    ...
    layui.form.on('submit(moire-submit)', function(data) {
        // 渲染数据
        tray['data'] = {};
        $.each(fxView['deploy']['cache']['search'], function(key, value) {
            // 执行输出
            value['echo']();
    ...
    layui.form.on('switch', function(event) {
        // 初始化变量
        var data = {};
        data[event['elem']['name']] = event['elem']['checked'] ? 1 : 0;
        // 处理数据
        fxView['store']['deal']({
    ...
    layui.table.on('toolbar(moire-table)', function(event) {
        ...
        switch (event.event) {
            ...
            case 'upload':
                ...
                fxView['mould']['tool']['window'](tray['param']);
                break;
            case 'delete':
                // 删除
                tray['param'] = {
                ...
                fxView['mould']['tool'][event.event](tray['param']);
                break;
            case 'import':
                // 导入
            case 'export':
