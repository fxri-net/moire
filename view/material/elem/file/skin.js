...
fxView['machine']['deployer'](['material', 'elem', 'file', 'skin', 'deploy'], {
  ...
  view: function (dark, base, echo, tray) {
    ...
    if (dark['delete']['switch']) {
      dark['delete']['elem'] =
        '<button class="layui-btn layui-btn-sm layui-btn-danger moire-bg-red-light moire-delete" type="button">' + dark['delete']['text'] + '</button>'
      // 点击事件
    ...
    if (dark['upload']['switch']) {
      ...
      // 疏理配置
      dark['upload'] = fxBase['param']['merge'](
        dark['upload'],
        {
          // 元素
        ...
        if (data['code'] == 200) {
          ...
          $.each(data['data'], function (key, value) {
            ...
            switch (dark['option']['type']) {
              ...
              case 'video':
                // 视频
                dark['elem'].find('.moire-elem-inline').append('<div><a href="' + value + '" target="_blank">点击链接</a>' + dark['delete']['elem'] + '</div>')
                break
              case 'image':
                // 图片
                dark['elem'].find('.moire-elem-inline').append('<div><img src="' + value + '"' + dark['alt'] + '>' + dark['delete']['elem'] + '</div>')
                break
            }
          })
          // 疏理隐藏图片值
        ...
        $.each(data, function (key, value) {
          ...
          switch (dark['option']['type']) {
            ...
            case 'video':
              // 视频
              dark['elem']
                .find('.moire-elem-inline')
                .append('<div><a href="' + value['ossUrl'] + '" target="_blank">点击链接</a>' + dark['delete']['elem'] + '</div>')
              break
            case 'image':
              // 图片
              dark['elem'].find('.moire-elem-inline').append('<div><img src="' + value['ossUrl'] + '"' + dark['alt'] + '>' + dark['delete']['elem'] + '</div>')
              break
          }
        })
        // 疏理隐藏图片值
